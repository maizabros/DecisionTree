...
double gini_clases(cuenta_datos_clases res){
...
void calculo_entropia_clases(datos * vect, double totalVivos, double entropias_clases[10][2]){
    ...
    entropias_clases[8][0] = entropia_C - gini_clases(res);
    ...
    entropias_clases[9][0] = entropia_C - gini_clases(res);
...
void crearArbolDecision(tipoArbolBin * a, datos * e, int tamano, double entMin, int cont, int contMax){
    ...
    if (tamano == 0  ||/* isnan(entropia_C) || entropia_C <= entMin || */cont == contMax){
        ...
        if (entropias_clases[clase_seleccionada][0] <= entMin  /*|| entropia_C <= entMin/**/ ||cont == contMax){
        //if(entropia_C <= entMin) printf("\033[31mMINENT = [%F]\033[0m\n",entMin);
        printf("\033[31mENTROPIA HOJA [%F]\033[0m\n",entropia_C);
